...
then
  ...
  then
    ...
    then
      ...
      echo $ELEMENT | while IFS=\| read ATOMIC_NUMBER ATOMIC_MASS MELT_POINT_C BOIL_POINT_C SYMBOL NAME TYPE
      do
        echo "The element with atomic number $ATOMIC_NUMBER is $NAME ($SYMBOL). It's a $TYPE, with a mass of $ATOMIC_MASS amu. $NAME has a melting point of $MELT_POINT_C celsius and a boiling point of $BOIL_POINT_C celsius."
      done
    fi
  else
